...
################################################################################
setwd('/Users/jiaming/Desktop/cancer_m6A_predictor/file_verification')
################################################################################
############################      Data Analysis      ############################
################################################################################
# Load Raw Data
...
Nsample <- data[c(1:33920),] # First 33920 rows

# Uncomment below for subsets of negative samples based on specific conditions:
# Example: Nsample[1-5]
# indx <- which(1 <= data$row_sums & data$row_sums <= 5)
# data <- data[,-26]
# Nsample <- data[indx]
# set.seed(1)
# Nsample <- sample(Nsample, 33920)

# Example: Nsample[6-15]
# indx <- which(6 <= data$row_sums & data$row_sums <= 15)
# data <- data[,-26]
# Nsample <- data[indx]
# set.seed(1)
# Nsample <- sample(Nsample, 33920)
...
Psample <- data[c(130692:134083),] # Last 3392 rows
...
# Generate Sequence Features for Nsample
...
# Generate Sequence Features for Psample
...
for (i in 1:10) {
  ...
  saveRDS(conservation, file = './conservation.model_deleted_1.rds')
  # Evaluate Model (Within-Training Evaluation: CV)
  conservation <- readRDS('./conservation.model_deleted_1.rds')
...
saveRDS(final_result, './final_result_deleted_1.rds')
